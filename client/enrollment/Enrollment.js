...
export default function Enrollment ({match}) {
    ...
    const markComplete = () => {
        if (!enrollment.lessonStatus[values.drawer].complete) {
            ...
            let updatedData = {};
            updatedData.lessonStatusId = lessonStatus[values.drawer].id;
    ...
    const imageUrl = enrollment.course._id 
                ? `/api/courses/photo/${enrollment.course._id}?${new Date.now().getTime()}`
                ...
                classes= {{paper: classes.drawerPaper}}>
                    ...
                    <List>
                        <ListItem button onClick= {selectDrawer(-1)} 
                            className= {values.drawer == -1 ? classes.selectedDrawer 
                                                            : classes.unselected}>
                                <ListItemTextIcon><Info /></ListItemTextIcon>
                                <ListItemText primary= {"Course Overveiw"} />
                        </ListItem>
                    </List>
                    <Divider />
                        ...
                        <ListItem>
                            <ListItemText primary= {
                                <div className= {classes.progress}>
                                    ...
                                    out of 
                                    <span>{enrollment.lessonStatus.length}</span>
                                    completed
                                </div>
                            }/>
                        </ListItem>
                ...
                {values.drawer == -1 &&
                    <Card className= {classes.card}>
                        <CardHeader 
                            ...
                            subheader= {<div>
                                <Link to= {"/user/" + enrollment.course.instructor._id} 
                                className= {classes.sub}>By{enrollment.course.instructor.name}</Link>
                                <span className= {classes.category}>{enrollment.course.category}</span>
                            </div>}
                            action= {totalComplete == enrollment.lessonStatus.length &&
                        ...
                        <div className= {classes.flex}>
                            <CardMedia className= {classes.media} image= {imageUrl} title= {enrollment.course.name}/>
                            <div className= {classes.details}>
                                <Typography variant= "body1" className= {classes.subheading}>
                                    {enrollment.course.descrption} <br/>
                                </Typography>
                            </div>
                        </div>
                        <Divider/>
                        <div>
                            <CardHeader 
                                ...
                                action = {auth.isAuthenticated().user && auth.isAuthenticated().user._id == enrollment.course.instructor._id && 
                                (<span className= {classes.action}>
        ...
        <Card className= {classes.card}>
            ...
            <CardActions>
                <a href= {enrollment.course.lessons[values.drawer].resource_url}>
                    <Button variant= "contained" color= "priamry">Resource Link</Button>
                </a>
            </CardActions>
        </Card></>
        )}
