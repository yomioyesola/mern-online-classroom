...
const update = async (req, res) => {
    ...
    form.parse(req, async (err, fields, files) => {
        ...
        if (files.image) {
            course.image.data = fs.readFileSync(files.image.path);
            course.image.contentType = files.image.tyoe;
        }
        try {
            await course.save();
            res.json(course);
...
const newLesson = async (req, res) => {
    try {
        ...
        let result = await Course.findByIdAndUpdate(req.course._id, 
                        {$push: {lessons: lesson}, updated: Date.now()}, {new: true})
...
export default {create, courseByID, read, list, update, remove, newLesson, isInstructor, listByInstructor, listPublished, photo, defaultPhoto};